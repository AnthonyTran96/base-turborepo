...
import type { DialogProps, TYPE_MESSAGE } from './type';
...
const ComponentDialog = forwardRef((_: unknown, ref) => {
  ...
  return (
    ...
    >
      <div className="duration-500">
        ...
        <div className="text-14 text-text-primary tablet:text-center mt-4 font-normal">
          {state.content || 'Content dialog: ....'}
        </div>
        {state.sub && (
          <div className="text-10 text-error-500 tablet:text-center mt-8">{state.sub}</div>
        )}

        <div
          className={`tablet:flex-col mt-24 flex flex-row-reverse gap-[10px] ${state.classNameFooter}`}
        >
