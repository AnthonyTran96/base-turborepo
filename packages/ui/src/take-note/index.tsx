  ...
  return (
    <div className={`w-full ${wrapperClassName}`}>
      ...
      <Input.TextArea
        ...
        {...restProps}
        className={`border-color-300 text-14 text-color-800 placeholder:text-color-600 disabled:!border-color-200 disabled:!bg-color-200 disabled:!text-color-800 [&:has(textarea:disabled)]:!border-color-200 [&:has(textarea:disabled)]:!bg-color-200 [&>textarea]:text-color-800 [&>textarea]:placeholder:text-color-600 focus:shadow-none [&:has(.ant-input-data-count)]:mb-20 [&:has(textarea:focus)]:shadow-none ${
          error
            ? '!border-error-500'
...
export function AppTakeNoteControl<T extends FieldValues>({
  ...
  return <AppTakeNote {...resField} {...restProps} error={error?.message} />;
}
